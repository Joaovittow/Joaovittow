  
### João Vitor Seixas Nunes 
Desenvolvedor FullStack 💻


---

### Sobre 🔎
Sou Desenvolvedor FullStack com experiência em JavaScript e TypeScript, construindo interfaces modernas no frontend e APIs RESTful robustas no backend. Trabalho com React, Node.js, Express e Fastify, e administro bancos PostgreSQL e MongoDB. No dia a dia, utilizo Docker e Git para garantir um fluxo de desenvolvimento eficiente, com foco em escalabilidade, desempenho e uma excelente experiência do usuário.

---

### Habilidades 🧰
- **Linguagens**: JavaScript, TypeScript
- **Frontend**: React, Next.js, HTML5, CSS3, Tailwind CSS
- **Backend**: Node.js, Express, Fastify
- **Banco de Dados**: PostgreSQL, MongoDB, Sequelize
- **Infra e DevOps**: Docker, Git
- **Boas Práticas**: API REST, autenticação JWT, versionamento, documentação

---


### Projetos em Destaque 🚀
- [DevBurger-Interface](https://github.com/Joaovittow/DevBurger-Interface) — Interface web moderna e responsiva para gestão completa de uma hamburgueria, feita com React e Vite. Inclui autenticação, catálogo, carrinho, checkout com Stripe e painel administrativo.
- [fullstackweek-barber](https://github.com/Joaovittow/fullstackweek-barber) — Aplicação fullstack para barbearia, com fluxo de agendamento e gestão de serviços.
- [finance-interface](https://github.com/Joaovittow/finance-interface) — Aplicação web para controle financeiro pessoal, com foco em receitas, despesas, períodos (meses/quinzenas) e categorias personalizadas.

---

### Contato 📫
- Email: [joaovitorseixasn13@outlook.com](mailto:joaovitorseixasn13@outlook.com)
- LinkedIn: [linkedin.com/in/joaovittow](https://www.linkedin.com/in/joaovittow/)

